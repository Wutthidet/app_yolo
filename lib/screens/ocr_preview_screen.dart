...
class _OcrPreviewScreenState extends State<OcrPreviewScreen>
  ...
  Future<void> _processImage() async {
    ...
    final detectionResults = await YoloService.detectObjectsOnImage(
        base64Image: widget.imageData.base64);

    if (detectionResults == null) {
      _showErrorDialog('ไม่สามารถตรวจจับวัตถุได้');
      setState(() {
        _isProcessing = false;
      });
      return;
    ...
    List<Future<OcrDetectionResult>> ocrFutures = [];
    for (var result in detectionResults) {
      ...
      ocrFutures.add(
        OcrService.recognizeTextFromCroppedImage(
          imageBytes: widget.imageData.bytes,
          boundingBox: rect,
        ).then((ocrText) => OcrDetectionResult(
    ...
    final List<OcrDetectionResult> ocrResults = await Future.wait(ocrFutures);
      ...
      Navigator.pushReplacement(
        ...
        PageRouteBuilder(
          pageBuilder: (context, animation, secondaryAnimation) =>
            ...
            ocrDetectionResults: ocrResults,
                        ...
                        children: [
                          ScaleTransition(
                            ...
                            child: ImagePreviewCard(
                              imageBytes: widget.imageData.bytes,
                              headerGradient: AppConstants.accentGradient,
                            ),
                          ),
